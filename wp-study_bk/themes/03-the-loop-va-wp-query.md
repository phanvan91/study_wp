...
<main class="site-main">
    ...
    <section class="main-posts">
        ...
        // Không cần wp_reset_postdata() cho Main Query
        ?>
    </section>
...
// Tạo ra HTML có class: nav-links, page-numbers, current, prev, next
...
function developer_include_custom_post_types( $query ) {
    if ( ! is_admin() && $query->is_main_query() ) {
        ...
        // Tag archive: bao gồm cả products
        if ( $query->is_tag() ) {
...
// === TẠI SAO KHÔNG DÙNG query_posts()? ===
// query_posts() GHI ĐÈ Main Query, gây ra:
// 1. Chạy query 2 lần (chậm)
// 2. Phá hỏng pagination
// 3. Ảnh hưởng đến conditional tags
// 4. Đã bị deprecated (lỗi thời)
...
// ĐÚNG: Dùng pre_get_posts hook
// Hoặc dùng new WP_Query cho secondary queries
```

---

## 9. Code ví dụ: Trang blog với nhiều loops
 ...
 *
 * Trang blog kiểu tạp chí với nhiều section khác nhau
 *
...
<main id="primary" class="site-main blog-magazine">
    ...
    <section class="magazine-hero">
        <div class="container">
            ...
            // Fallback: Nếu không có bài hero, lấy bài mới nhất
            if ( ! $hero->have_posts() ) {
                $hero = new WP_Query( array(
                    'post_type'      => 'post',
                    'posts_per_page' => 1,
            ...
            if ( $hero->have_posts() ) :
                $hero->the_post();
                $hero_id = get_the_ID(); // Lưu ID để loại trừ sau
            ?>
    ...
    <!-- ===== TRENDING: 4 bài viết nhiều lượt xem ===== -->
    <section class="magazine-trending">
        <div class="container">
            <h2 class="section-title">
                <span class="title-icon">&#128293;</span>
                <?php esc_html_e( 'Đang Xu Hướng', 'developer-theme' ); ?>
            </h2>
            ...
            $trending = new WP_Query( array(
                ...
                'order'          => 'DESC',
                'date_query'     => array(
                    array( 'after' => '7 days ago' ), // Chỉ trong 7 ngày qua
                ),
            ) );

            // Fallback nếu không có meta views
            ...
            $trending_ids = array( $hero_id ); // Mảng ID đã hiển thị
        ...
        <div class="container">
            <div class="content-area">
                ...
                <div class="main-content">
                    <h2 class="section-title"><?php esc_html_e( 'Bài Viết Mới', 'developer-theme' ); ?></h2>
    ...
    <section class="magazine-categories">
        <div class="container">
            <?php
            $featured_cats = array( 'cong-nghe', 'kinh-doanh', 'doi-song' ); // Slug của các danh mục
            ...
            ?>
                <div class="category-block">
                    <div class="category-header">
                        ...
                        <a href="<?php echo esc_url( get_category_link( $cat->term_id ) ); ?>" class="view-all">
                            <?php esc_html_e( 'Xem tất cả', 'developer-theme' ); ?> &rarr;
                        </a>
                    </div>
                    ...
                    <div class="category-posts">
                        ...
                        <?php
                                ...
                                // Các bài còn lại: hiển thị danh sách
                        ?>
...
## 10. So sánh với Eloquent trong Laravel
...
// Delete
wp_delete_post( 42 );         // Chuyển vào thùng rác
wp_delete_post( 42, true );   // Xóa vĩnh viễn
```
...
### 1. Luôn reset postdata

```php
// Sau MỖI custom WP_Query, GỌI wp_reset_postdata()
$query = new WP_Query( $args );
while ( $query->have_posts() ) : $query->the_post();
    // ...
endwhile;
wp_reset_postdata(); // BẮT BUỘC!

// Không cần reset cho Main Query
// Không cần reset cho get_posts() (vì get_posts không thay đổi global $post)
```

### 2. Dùng pre_get_posts thay vì query_posts

```php
// SAI: query_posts() - KHÔNG BAO GIỜ dùng
...
```php
// Khi chỉ cần IDs (ví dụ: đếm số, hoặc truyền cho post__in)
$ids = new WP_Query( array(
    'fields'         => 'ids',           // Chỉ lấy IDs, không lấy toàn bộ object
    'no_found_rows'  => true,            // Không chạy SQL_CALC_FOUND_ROWS (nhanh hơn)
    'posts_per_page' => 100,
) );

// Khi không cần meta hoặc terms
$query = new WP_Query( array(
    'update_post_meta_cache' => false,   // Không preload meta
    'update_post_term_cache' => false,   // Không preload terms
) );

// Tránh orderby rand trên dataset lớn (chậm)
// Thay vì:
...
```php
// LUÔN escape khi echo ra HTML
echo esc_html( get_the_title() );           // Text bình thường
echo esc_url( get_the_permalink() );         // URL
echo esc_attr( get_the_title() );            // HTML attribute
echo wp_kses_post( get_the_content() );      // HTML nội dung (cho phép 1 số tag)

// Các hàm the_*() đã tự escape
// Các hàm get_the_*() cần escape khi echo
```

### 5. Dùng get_posts() cho query đơn giản

```php
// get_posts() phù hợp cho:
// - Lấy nhanh danh sách bài viết
// - Không cần pagination
// - Không cần thay đổi global $post

$recent_posts = get_posts( array(
    'numberposts' => 5,              // Lưu ý: 'numberposts', KHÔNG phải 'posts_per_page'
    'post_type'   => 'post',
    'post_status' => 'publish',
) );

foreach ( $recent_posts as $post ) :
    // Dùng $post->title thay vì the_title()
    echo '<li><a href="' . get_permalink( $post ) . '">' . esc_html( $post->post_title ) . '</a></li>';
endforeach;
// Không cần wp_reset_postdata() vì get_posts() KHÔNG thay đổi global $post
```

---

**Tiếp theo:** [04 - Menus, Widgets, Sidebars](./04-menus-widgets-sidebars.md) - Tạo menu, widget, và sidebar cho theme
