<?php
/**
 * Thiết lập các bộ lọc và hành động mặc định cho Multisite.
 *
 * Nếu bạn cần gỡ bỏ một hook mặc định, tệp này sẽ cung cấp cho bạn mức ưu tiên
 * để sử dụng khi gỡ hook.
 *
 * Không phải tất cả các hook mặc định của Multisite đều được tìm thấy trong ms-default-filters.php
 *
 * @package WordPress
...
// Các bộ đếm này được xử lý bởi wp_update_network_counts() trên Multisite:
...
// Vô hiệu hóa một số thứ mặc định cho multisite.
add_filter( 'enable_update_services_configuration', '__return_false' );
if ( ! defined( 'POST_BY_EMAIL' ) || ! POST_BY_EMAIL ) { // Hằng số tương thích ngược.
	add_filter( 'enable_post_by_email_configuration', '__return_false' );
}
if ( ! defined( 'EDIT_ANY_USER' ) || ! EDIT_ANY_USER ) { // Hằng số tương thích ngược.
	add_filter( 'enable_edit_any_user_configuration', '__return_false' );
}
add_filter( 'force_filtered_html_on_import', '__return_true' );

// WP_HOME và WP_SITEURL không có hiệu lực trong MS.
remove_filter( 'option_siteurl', '_config_wp_siteurl' );
remove_filter( 'option_home', '_config_wp_home' );

// Một số thay đổi tùy chọn nên kích hoạt làm mới chi tiết site.
...
// Nếu nâng cấp mạng chưa chạy, giả định rằng viết lại ms-files.php được sử dụng.
add_filter( 'default_site_option_ms_files_rewriting', '__return_true' );

// Cho phép các tên miền multisite cho các yêu cầu HTTP.
